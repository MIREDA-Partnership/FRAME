...
# has drugs recorded labetalol and nifedipine 
...
res_df <- res_df %>% bind_rows(data.frame(item='number of pregnancies aged 18+ with labetalol or nifedipiner code ever recorded ',item_value =nrow(preg_18_reporting_df %>% filter(l_n_drugs_recorded ==TRUE))))
res_df <- res_df %>% bind_rows(data.frame(item='number of pregnancies aged 18+ with labetalol or nifedipiner code recorded during pregnancy',item_value =nrow(preg_18_reporting_df %>% filter(l_n_drugs_in_preg==TRUE))))
res_df <- res_df %>% bind_rows(data.frame(item='number of pregnancies aged 18+ with labetalol or nifedipiner code recorded before pregnancy',item_value =nrow(preg_18_reporting_df %>% filter(l_n_drugs_before_preg==TRUE))))
res_df <- res_df %>% bind_rows(data.frame(item='number of pregnancies aged 18+ with with hypertensive disorder code ever recorded and  labetalol or nifedipiner code recorded during pregnancy',item_value =nrow(preg_18_reporting_df %>% filter(frame_cohort==TRUE))))
