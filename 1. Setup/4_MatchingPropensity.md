...
- [Setup](#setup)
- [Propensity table](#propensity-table)
- [Fit logistic regression (propensity score
  model)](#fit-logistic-regression-propensity-score-model)
- [Split dataset](#split-dataset)
- [Exact matching with nearest propensity
  score](#exact-matching-with-nearest-propensity-score)
- [Iterative matching process](#iterative-matching-process)
- [Bind the list](#bind-the-list)
- [Merge back to original characteristics for final
  review](#merge-back-to-original-characteristics-for-final-review)
- [Export data](#export-data)
- [Get matched counts for google
  doc](#get-matched-counts-for-google-doc)
- [Clean up](#clean-up)

## Setup

If you are not already connected to your FRAME cohort, this should do it
for you. Check that the *db_path* is correct before running it.
*getwd()* will return the directory that you have saved this file to so
if your DuckDB is in a different directory, change it first.
...
The outcome is the *frst_preg_drug* (binary) representing choosing
nifedipine over the first line treatment of labetalol
...
Need to split into the intervention group frst_preg_drug == ‘nifedipine’
and the control group *frst_preg_drug* == ‘labetalol’.
...
```
    ...
    Ncells 1853696 99.0    3589628 191.8  2535197 135.4
    Vcells 3261061 24.9    8388608  64.0  8025672  61.3
