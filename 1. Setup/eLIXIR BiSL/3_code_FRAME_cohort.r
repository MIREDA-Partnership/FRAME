...
req_pkgs <- c("DBI", "duckdb",  "tidyverse", "dbplyr", "dtplyr",
              "data.table", "bit64","writexl")

invisible(suppressPackageStartupMessages(
  lapply(req_pkgs, library, character.only = TRUE, logical.return = FALSE)))
rm(req_pkgs)
...
diabpreg <- get_descendants(cdm, 4058243L) %>%
...
diabetes <- get_descendants(cdm, 201820L) %>% 
  select(concept_id = descendant_concept_id) %>% 
  union_all(cdm$concept %>% 
...
respiratory <- get_descendants(cdm, 317009) %>% 
  union_all(get_descendants(cdm, 42873168)) %>% 
  union_all(get_descendants(cdm, 4063381)) %>% 
  union_all(get_descendants(cdm, 974952)) %>% 
  union_all(get_descendants(cdm, 1340232)) %>% 
  union_all(get_descendants(cdm, 4224259)) %>% 
  union_all(get_descendants(cdm, 42539027)) %>% 
  union_all(get_descendants(cdm, 313459)) %>% 
  union_all(get_descendants(cdm, 4044240)) %>% 
  union_all(get_descendants(cdm, 43531585)) %>% 
  union_all(get_descendants(cdm, 37110295)) %>% 
  union_all(get_descendants(cdm, 40483220)) %>% 
  union_all(get_descendants(cdm, 4247108)) %>% 
  union_all(get_descendants(cdm, 317971)) %>% 
  union_all(get_descendants(cdm, 4005302)) %>% 
  union_all(get_descendants(cdm, 196724)) %>% 
  union_all(get_descendants(cdm, 37395588)) %>% 
  union_all(get_descendants(cdm, 4049972)) %>% 
  union_all(get_descendants(cdm, 4028286)) %>% 
  union_all(get_descendants(cdm, 4060429)) %>% 
  select(concept_id = descendant_concept_id) %>% 
  union_all(cdm$concept %>% 
              filter(
...
hypertension <- get_descendants(cdm, 316866L) %>% 
  union_all(get_descendants(cdm, 4086587L)) %>%
  select(concept_id = descendant_concept_id) %>% 
  union_all(cdm$concept %>% 
...
preeclampsia <- get_descendants(cdm, 439393L) %>% 
  select(concept_id = descendant_concept_id) %>% 
  union_all(cdm$concept %>% 
...
# eclampsia
# 443700
eclampsia <- get_descendants(cdm, 443700L) %>% 
  select(concept_id = descendant_concept_id) %>% 
  union_all(cdm$concept %>% 
...
cvd <- get_descendants(cdm, 134057L) %>%
  select(concept_id = descendant_concept_id) %>% 
  union_all(cdm$concept %>% 
...
prem_labor <- get_descendants(cdm, 4273560L) %>%
  select(concept_id = descendant_concept_id) %>% 
  union_all(cdm$concept %>% 
...
cervical_incomp <- get_descendants(cdm, 436740L) %>%
  select(concept_id = descendant_concept_id) %>% 
  union_all(cdm$concept %>% 
              filter(concept_id == 436740L) %>% 
              select(concept_id))

pprom <- get_descendants(cdm, 4208215L) %>% 
  select(concept_id = descendant_concept_id) %>% 
  union_all(cdm$concept %>% 
...
writexl::write_xlsx(
  x = exclusions,
  path = file.path(frame_db_folder , exclusion_file )
)
  ...
  mutate(
    ...
    inc_gpandas_periods = if_else(
      between(frst_preg_drug_strt_dt,
              as.Date("2018-06-08"), as.Date("2025-01-17")), 1L, 0L
    )) %>% 
...
writexl::write_xlsx(
  ...
  path = file.path(frame_db_folder , unmatched_file )
)
